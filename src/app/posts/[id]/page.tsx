...
export default function PostDetail({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchData = async () => {
      try {
        ...
        const [postRes, usersRes] = await Promise.all([
          fetch(`https://jsonplaceholder.typicode.com/posts/${parsedPostId}`),
        ...
        const authorData = users[postData.id % users.length];
  ...
  if (loading) {
    return <Status type="loading" message="Loading car blog details..." spinnerSize="lg" />;
  }
  ...
  if (error || !post || !postId) {
    return (
      <Status 
        type="error" 
        message="Failed to load car blog details" 
        onRetry={handleRetry}
      />
    );
  }
