...
const NewTechnology = () => {
  ...
  const totalSegments = Math.ceil(techPosts.length / visibleCount);

  const handleNext = () => setCurrentIndex(prev => Math.min(prev + visibleCount, maxIndex));
  const handlePrev = () => setCurrentIndex(prev => Math.max(prev - visibleCount, 0));

  if (loading || error || techPosts.length === 0) {
    return (
      <section className="max-w-6xl mx-auto mt-10 px-4">
        ...
        <Status
          ...
          message={loading ? 'Loading technology posts...' : error ? 'Unable to fetch technology posts.' : 'No technology posts available.'}
        />
      </section>
    );
  }
  ...
  return (
    <section className="max-w-6xl mx-auto mt-10 px-4">
      ...
      <div className="relative">
        ...
        <div className="flex justify-center mt-4 overflow-x-auto scrollbar-hide">
